    ...
    <div className="relative min-h-screen overflow-hidden bg-[#05060A] text-slate-100">
      ...
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#05060A]/70 backdrop-blur-xl">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-slate-400">ConsenTide</p>
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium text-slate-300 md:flex">
            ...
            <Link className="transition hover:text-white" href="/api-docs">
              API Docs
            </Link>
          </nav>
          ...
          <Link
            href="/dashboard"
            className="group hidden items-center gap-2 rounded-full border border-white/20 bg-white/5 px-4 py-2 text-sm font-semibold text-white shadow-[0_10px_30px_rgba(32,40,72,0.45)] backdrop-blur xl:flex"
          >
            Launch app
      ...
      <footer className="border-t border-white/10 bg-black/30 py-8 backdrop-blur">
        <div className="mx-auto flex max-w-6xl flex-col gap-2 px-6 text-sm text-slate-400 md:flex-row md:items-center md:justify-between">
          <p>© {new Date().getFullYear()} ConsenTide • Built for Constellation x LegalTech Hackathon</p>
    ...
    <section className="relative mx-auto flex max-w-6xl flex-col gap-16 px-6 pb-12 pt-20 lg:flex-row lg:items-center lg:justify-between lg:pt-28">
      <div className="relative z-10 max-w-xl space-y-8">
        ...
        <h1 className="text-4xl font-semibold leading-tight text-white sm:text-5xl lg:text-6xl">
          Consent intelligence for the <span className="bg-gradient-to-r from-sky-400 via-violet-500 to-pink-400 bg-clip-text text-transparent">hypergraph era</span>
        </h1>
        <p className="text-lg text-slate-300">
          Grant, verify, and revoke GDPR consent with cinematic precision. Zero-knowledge proofs, immutable audit trails,
          and governance powered by El Paca—all streaming through a single command center.
        </p>
      ...
      <div className="relative flex w-full max-w-lg flex-col gap-5 rounded-3xl border border-white/10 bg-white/5 p-8 shadow-[0_40px_120px_rgba(12,22,48,0.45)] backdrop-blur lg:translate-y-6">
        <span className="text-xs uppercase tracking-[0.45em] text-slate-400">Live telemetry</span>
        ...
        <div className="rounded-2xl border border-white/10 bg-gradient-to-r from-white/10 to-white/0 p-6">
          <p className="text-sm text-slate-200">
            “ConsenTide is the only consent fabric delivering zk-proof attestations, programmable governance, and HGTP anchoring
            without sacrificing developer velocity.”
          </p>
          <p className="mt-4 text-xs uppercase tracking-[0.3em] text-slate-400">Constellation integration team</p>
        </div>
      </div>
    </section>
  )
    ...
    <section className="mx-auto max-w-6xl px-6 py-24">
      <div className="mb-12 flex flex-col gap-4 text-center">
        ...
        <p className="mx-auto max-w-3xl text-base text-slate-300">
          From dynamic consent journeys to cross-chain attestations, ConsenTide delivers the building blocks to move from
    ...
    <section className="relative mx-auto max-w-6xl overflow-hidden rounded-4xl border border-white/10 bg-gradient-to-br from-white/10 via-white/5 to-transparent px-8 py-16 backdrop-blur-xl">
      ...
      <div className="relative flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
        <div className="max-w-xl space-y-6">
          ...
          <p className="text-sm text-slate-200">
            Deploy ConsenTide in hours, not months. Use Supabase migrations, Docker blueprints, and our SDKs to orchestrate
